...
func promptStrategyFor(typ GenerationType) generationPromptStrategy {
	...
	case GenerationTypePPT:
		return generationPromptStrategy{
			...
			OutputFormat: "仅返回 HTML 片段，不要返回 Markdown 大纲，不要输出 <PPT_FILE>，不要输出 <PREVIEW_LINK>，不要伪造下载链接或预览链接。必须逐项覆盖 STRUCTURED_PPT_PLAN：计划里有多少个 Slide，最终就必须输出多少个 <section>；每个计划页都要有同名或明显对应的 h1/h2 标题。每页必须使用一个 <section>；每个 <section> 必须按固定 PPT 画布设计：width: 1920px; height: 1080px; box-sizing: border-box; overflow: hidden; 推荐 section padding: 80-120px。必须使用适合 16:9 PPT 导出的显式 font-size：封面 h1: 76-96px，页面标题 h2: 48-64px，小标题 h3: 34-44px，正文 p/li: 30-38px，注释和脚注不低于 24px；不要使用网页卡片式 max-width: 1100px 或 1rem 级小字号作为主版式。可包含 <style>，但不要包含 html/body 外层标签。HTML 必须适合转换为可编辑 PPT：使用 h1/h2/h3/p/ul/ol/li/div/span/strong/em 等语义元素；使用 CSS 表达背景、边框、圆角、padding、margin、grid/flex、字号、字重、行高、文字颜色和对齐。避免复杂脚本、外链字体、侵权图片、过度动画和无法导出的 CSS。每页聚焦一个主题，正文以 3-5 个要点为宜，必要补充必须明确标记为解释或补充，不得当作已有来源事实。",
		}
...
func pptOutlinePromptStrategy() generationPromptStrategy {
	return generationPromptStrategy{
		System:       "先执行内容结构分析，再执行 outline_plan 和 content_expand，生成内部 PPT 详细大纲。以用户 Markdown 或 <PPT_CONTENT> 为主，完整提取标题层级、正文段落、列表、图片描述、证据和用户意图；再结合 <PPT_STYLE> 或用户提示确定风格方向。必须根据材料规模动态决定页数：短材料生成 8-10 页，中等材料生成 10-14 页，长材料生成 14-20 页。逐页写清楚页面目的、核心论点、关键要点和可用证据；不要把多个二级标题强行合并到同一页。检索和联网内容只作支持；稀疏输入可以补充学习解释，但必须标记为解释补充，不能伪装成来源事实。",
		OutputFormat: "仅返回 Markdown，不要返回 HTML。包含标题，并包含封面页、目录页、内容页、结束页；动态决定页数，整体建议 8-20 页。逐页写清楚：页面标题、页面目的、3-5 个要点、可使用的证据或来源提示。内容页优先按 Markdown 的二级/三级标题组织；如果一个章节内容很多，可以拆成多页。不要输出参考资料或 References 页面。",
	}
}
